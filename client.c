...
int netopen(const char *pathname, int flags){
	...
	if (flags == O_RDONLY){
		...
		sprintf(buffer, "OPEN %s %s w ", fmode, pathname);
		sprintf(flag, "w");
	}
...
int main(int argc, char* argv[]){
	...
	char message[200];
	strcpy(message, "Malik loves dick\0");
	...
	int testfd = netopen("test1.txt", O_RDWR);
	printf("Testfd from Open: %d\n", testfd);
	
	char test[200];
	int testread = netread(testfd, test, 200);
	if(testread < 0){
		printf("ERROR: Reading was not compeleted \n");
	}else{
		printf("%d bytes were read from testfd. \n", testread);
	}
	
	char testmessage[200];
	stpcpy(testmessage, "testing netclose \n");
	int testwrite = netwrite(testfd, testmessage, strlen(testmessage));
	if(testwrite < 0){
		printf("ERROR: Writing was not completed. \n");
	}else{
		printf("%d bytes were written into testfd. \n", testwrite);
	}
	int testclose = netclose(testfd);
	if(close < 0){
		printf("ERROR: Close was not completed. \n");
	}else{
		printf("Testfd was closed.\n");
	}*/

	printf("Everything works MOTHAFUCKA!!!\n");
	return 0;
}